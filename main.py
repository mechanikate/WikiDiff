...
def compare(dataarg: List[str], start: int=0, end: int=3, minimum_prevalence: int=1) -> List[str]:
    ...
    dupes = [item for item, count in collections.Counter(curr).items() if count > int(minimum_prevalence)] # If text is seen more than minimum_prevalence times, then add it into dupes
...
@app.route('/controversialness') # https://example.com/controversialness?article=...&minprev=...&articleno=...
def controversialness_page(): # Literally just returns a percent of what amount of the text has changed over 16:07
    if(request.args.get("minprev") in ["", None] or request.args.get("articleno") in ["", None] or request.args.get("article") in ["", None]):
        return "0%"
